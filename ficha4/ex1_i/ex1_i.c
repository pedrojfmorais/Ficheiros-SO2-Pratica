...
int _tmain(int argc, TCHAR* argv[]) {
	...
	hTrinco = CreateMutex(NULL, FALSE, NULL);

	for (DWORD i = 0; i < N; i++)
	{
		dados[i].somatorio = 0;
		dados[i].trinco = &hTrinco;
		dados[i].numSomados = &numSomados;
	...
	while (n > 0) {
		DWORD res = WaitForMultipleObjects(n, hThread, FALSE, 1000 * 60 * 5);

		if (res == WAIT_TIMEOUT) {
			_tprintf(_T("Ainda à espera!\n"));
			continue;
		}
		...
		_tprintf(_T("Acabou uma thread %d! Pretende continuar? (y/n)"), ids[res - WAIT_OBJECT_0]);
		_tscanf_s(_T("%c"), &r);
		...
		for (DWORD i = res - WAIT_OBJECT_0; i < n; i++) {
