...
LRESULT CALLBACK TrataEventos(HWND hWnd, UINT messg, WPARAM wParam, LPARAM lParam) {
	...
	RECT rect;
	PAINTSTRUCT ps;
	static TCHAR teclaCarregada = '?';
	static PosicaoChar posChar[1000];
	static int indexPosChar = 0;
	switch (messg) {
	case WM_CLOSE:
	...
	case WM_PAINT:
		...
		for (DWORD i = 0; i < indexPosChar; i++)
		{
			rect.left = posChar[i].xPos;
			rect.top = posChar[i].yPos;
			DrawText(hdc, &posChar[i].tecla, 1, &rect, DT_SINGLELINE | DT_NOCLIP);
		}
		EndPaint(hWnd, hdc);
		break;

	case WM_CHAR:
		teclaCarregada = (TCHAR)wParam;
		break;
